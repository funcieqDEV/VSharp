...
namespace VSharp
{
    ...
    public class Interpreter 
    {
        ...
        {
            string importPath = EvaluateExpression(stmt.name, vars) as string ?? throw new Exception("Expected string path");

        
            string fullPath = Path.Combine("C:\\ProgramData\\VSharp\\libs", $"{importPath}.dll");
            ...
            if (stmt.name != null)
            {
                vars.SetVar(stmt.NameSpace, libraryExports);
            }
            else
            {
